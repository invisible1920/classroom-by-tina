...
import { ArrowLeft, BookOpen, Clock, Download, Sparkles, Star } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from("resources").select("id", { count: "exact", head: true }).eq("status", "published").eq("featured", true),
    supabase.from("resource_downloads").select("id", { count: "exact", head: true }).eq("user_id", user.id),
    supabase.from("resources").select("id", { count: "exact", head: true }).eq("status", "published").gte("created_at", thirtyDaysAgo),
  ]);
    ...
    <>
      {profile.role === "admin" && (
        <div className="mb-6 flex justify-end">
          <Link href="/admin" className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-5 py-2.5 font-semibold text-slate-700 shadow-sm transition hover:border-blue-600 hover:text-blue-600">
            <ArrowLeft size={18} />
            Back to Admin
          </Link>
        </div>
      ...
      <section className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        <StatsCard title="Resources" value={String(resourceCount ?? 0)} subtitle="Available to you" icon={<BookOpen size={24} />} />
        <StatsCard title="Featured" value={String(featuredCount ?? 0)} subtitle="Recommended resources" icon={<Star size={24} />} />
        <StatsCard title="New" value={String(recentlyAddedCount ?? 0)} subtitle="Added this month" icon={<Clock size={24} />} />
        <StatsCard title="My Downloads" value={String(myDownloadCount ?? 0)} subtitle="Resources downloaded" icon={<Download size={24} />} />
      </section>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-4 text-3xl font-black tracking-tight text-[#17223b]">
              Plan faster. Teach beautifully.
            </h2>
            <p className="mt-2 max-w-2xl text-base font-medium leading-7 text-slate-600">
              Classroom by Tina AI helps CMS K–2 teachers turn skills, weeks, and classroom needs into polished planning support.
            </p>
          </div>

          <div className="rounded-3xl bg-[#fffaf3] px-5 py-4 text-sm font-black text-[#17223b]">
            ✨ Built for Charlotte K–2 classrooms
          </div>
        </div>
  ...
  >
    ...
    <div className="mt-5 inline-flex items-center rounded-full bg-white px-4 py-2 text-sm font-black text-[#35c6c9] shadow-sm transition group-hover:translate-x-1">
      ...
      <section className="mt-12">
        <SectionTitle eyebrow="Teaching Library" title="Browse by Grade" description="Start with the grade level, then move into subject, month, week, and resource type." />

        <div className="mt-6 grid gap-6 md:grid-cols-3">
          {gradeLinks.map((grade) => (
            <GradeCard key={grade.name} name={grade.name} href={grade.href} description={grade.description} />
          ))}
        </div>
      </section>

      <section className="mt-12">
        <SectionTitle eyebrow="Recommended" title="Featured Resources" description="High-priority resources selected for teachers to see first." />

        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {featuredResources.items.map((resource) => (
            <ResourceCard key={resource.id} resource={resource} isFavorite={favoriteResourceIds.has(resource.id)} />
          ))}
        </div>
      </section>

      <section className="mt-12">
        <SectionTitle eyebrow="Recently Added" title="New Teaching Materials" description="Freshly added resources for planning, centers, and classroom instruction." />
